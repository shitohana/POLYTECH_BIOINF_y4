...
`-m` - можно указать **protein** - тогда аннотация будет не по геному, а протеому, однако тогда для `-i` надо указать *файл-fasta* с белками

**НО такой вариант работает только с VPN или уже скачанными базами lineage**


## BUSCO {auto-animate="true"}

При первых запусках новые lineage будут скачиваться локально. Но этого не проивойдет без VPN...
...
C:11.2% - **completeness** (потому что мы аннотируем одну из хромосом)
